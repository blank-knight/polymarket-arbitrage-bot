...
class Polymarket:
    def __init__(self) -> None:
        ...
        self.chain_id = 137  # 告诉客户端你在哪个网跑（Polygon 主网 ID 是 137）
        self.private_key = os.getenv("POLYGON_WALLET_PRIVATE_KEY")
        self.polygon_rpc = "https://polygon-rpc.com"
        # Web3:以太坊（以及兼容链如 Polygon）的 Python 接口库。HTTPProvider通过 HTTP/HTTPS 连接到区块链节点
        self.w3 = Web3(Web3.HTTPProvider(self.polygon_rpc))

        # 处理二元市场合约
        self.exchange_address = "0x4bfb41d5b3570defd03c39a9a4d8de6bd8b8982e"
        # 处理多个互斥选项市场的合约（这些地址一般通过官方文档或者github获取）
        self.neg_risk_exchange_address = "0xC5d563A36AE78145C45a50134d48A1215220f80a"

        # ABI（Application Binary Interface）
        # 合约的接口定义，用于与合约交互
        # self.exchange_abi = self.get_contract_abi(self.exchange_address)
        # self.neg_risk_exchange_abi = self.get_contract_abi(self.neg_risk_exchange_address)
        ...
        # usdc合约地址，通俗来说就是银行（大账本），你去这个地址查询余额，就是你的usdc余额。其遵循的erc20，同质化代币标准
        # 补充：ERC-721，非同质化代币标准，用于NFT
        self.usdc_address = "0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174"
        # CTF(Conditional Token Framework，条件代币框架):在pm上购买的，其实是一种条件代币。当你买入一个市场时，你的 USDC 会被锁定。
        # ctf_address 合约会根据你的选择，凭空为你“铸造”出对应的 Yes 份额或 No 份额。这个合约地址记录了谁持有哪个市场的份额、持有多少。
        # 其遵循的erc-1155,多功能混合型标准，适用于游戏、元宇宙等复杂场景。例如，一个合约中既可以包含游戏内货币，又能管理角色或装备类 NFT。
        self.ctf_address = "0x4D97DCd97eC945f40cF65F87097ACe5EA0476045"

        # 创建连接Polygon 区块链的对象
        self.web3 = Web3(Web3.HTTPProvider(self.polygon_rpc))
        # 注入POA中间件，用于处理Polygon网络的特殊情况（采用PoA (Proof of Authority) 或其变种共识机制，它需要把许多验证者的签名信息塞进这个 
        # extraData 字段里)
        self.web3.middleware_onion.inject(geth_poa_middleware, layer=0)

        # 这里传入具体的合约地址和abi，创建合约对象
        # 合约对象可以用来调用合约的函数，查询合约的状态，监听合约的事件等
    ...
    def _init_api_keys(self) -> None:
        ...
        # Polymarket 不希望你每次发订单都直接用私钥去签（因为私钥太敏感）。
        # 逻辑：它利用你的私钥生成一套专用的 L2 API Key。这套 Key 只负责下单、撤单，就像是给你的私钥办了一个“临时通行证
        self.credentials = self.client.create_or_derive_api_creds() 
        # 将其和客户端把绑定起来，后续所有操作都基于这套 Key 进行   
    ...
    def _init_approvals(self, run: bool = False) -> None:
        ...
        # CTF Exchange
        # 给usdc合约授权，令其能够直接快速操作我的usdc（例如，当我在pm上买yes时，需要用usdc支付，而usdc合约需要知道我是谁，才能给我转账）
        # 拿到合约
    ...
    def get_all_events(self) -> "list[SimpleEvent]":
        events = []
        # 和request相比，异步请求更快。request适合同步场景
        ...
        if res.status_code == 200:
            ...
            for event in res.json():
                try:
                    ...
                    # **event_data用于解包，从而将event_data中的键值对作为参数传递给SimpleEvent
    ...
    # 过滤处于激活状态，并且未被限制（合规），未被归档（过期很久），未被关闭（已经结束）的事件
        tradeable_events = []
        for event in events:
            if (
                event.active
    ...
    def execute_market_order(self, market, amount) -> str:
        # TODO: 这里假设市场只有两个 outcome，分别是 yes 和 no。如果市场有更多 outcome，需要修改这里的逻辑
        ...
        # 采用FOK订单模式，Fill Or Kill（全额成交或立即取消）。
    ...
    def get_usdc_balance(self) -> float:
        ...
        # 就像人民币最小单位是“分”，USDC 也有它的最小单位。其中，Subunit是最小单位的统称，Wei是eth的最小单位
        # 18 精度 vs 6 精度: 大多数代币（如 ETH, MATIC）使用 18 位精度，但 USDC 和 USDT 常用 6 位精度。
        # SOL最小单位为Lamport精度为10^9
